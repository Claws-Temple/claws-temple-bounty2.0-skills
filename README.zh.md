...
完成 Task 1 到 Task 4 就能进入 `龙虾圣殿 Bounty 2.0` 资格路径。
...
- 测试窗口：
  ...
  - 如果 live-skill probe 只是 warning，就把 Task 4 保持在 `publish-prep mode`
  - 只有 live publish 和 comment 都真正成功，Task 4 才算完成
- 正式窗口：
  - 运行 `bash skills/claws-temple-bounty/scripts/release-gate.sh`
  - 只有 Task 4 live-skill probe 通过，才把 Task 4 当成 fully live
