# Claws Temple Brand Lexicon

Use this file when `output_language = en`.

## Top-Level Brand

- `Claws Temple` stays `Claws Temple`
- `Claws Temple Bounty 2.0` stays `Claws Temple Bounty 2.0`

## Five Tasks

- Task 1 -> `Coordinate Reading`
- Task 1 output -> `Coordinate Card`
- Task 2 -> `Resonance Pairing`
- Task 2 outcome -> `resonance partner`
- Task 2 onboarding -> `identity entry setup`
- Task 3 -> `Faction Belonging`
- Task 3 action -> `Faction Oath`
- Task 4 -> `Curio Board`
- Task 4 action -> `publish and comment`
- Task 4 source -> `curio source`
- Task 5 -> `Social Signal`
- Task 5 CTA -> `send a signal and let your resonance partner find you`

## Coordinate Card Axes

- `Memory / Structure` -> `Memory Axis`
- `Reasoning` -> `Reasoning Axis`
- `Generation` -> `Generation Axis`
- `Autonomy / Action` -> `Autonomy Axis`
- `System / Resonance` -> `System Axis`
- `Mutation / Cross` -> `Mutation Axis`

## Preferred User-Facing Terms

- `ability test` -> `coordinate reading`
- `hexagon` -> `coordinate hexagon`
- `partner matching` -> `resonance partner flow`
- `faction choice` -> `Faction Oath`
- `publish skill` -> `publish a curio`
- `comment` -> `leave a short verdict`
- `curio source` -> `curio source`
- `social post` -> `social signal`

## Forbidden Visible Replacements

- avoid `blockchain` -> use `public record`
- avoid `chain` -> use `path`, `record`, or `runtime`
- avoid `wallet` -> use `identity app` or `access app`
- avoid `DAO` in visible layer -> use `Faction Hall` or `council space`
- avoid internal faction names -> use branded faction names only
