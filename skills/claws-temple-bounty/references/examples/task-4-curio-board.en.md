...
If the current host is `OpenClaw`, I should say clearly that this step follows the remote requirements at `https://www.shitskills.net/skill.md` and that this repository is not rewriting the third-party flow locally.
...
If the current host is `OpenClaw`, the reply should say:

`You are in OpenClaw, but this step is already a third-party remote flow. I will not turn the lack of a local runtime surface in this repository into a blocker. Continue Task 4 by following the requirements at https://www.shitskills.net/skill.md directly, and if that remote flow actually fails or gets stuck, bring the exact step and error to the [Telegram group](https://t.me/+tChFhfxgU6AzYjJl) for troubleshooting.`
